...
class MlHandler(object):
    ...
    def replace_by_median(self, old, columns=None, inplace=False):
        ...
        print(columns, median.index.tolist())
    ...
    def scale(self, columns=None, inplace=False):
        if not inplace:
            new_df = copy(self.source)
        else:
            new_df = pd.DataFrame()
...
# ml_handler = MlHandler(pd.read_csv('Consumo_cerveja.csv'))
#
# # ml_handler.source.AvrTemp = pd.to_numeric(ml_handler.source.AvrTemp)
#
#
# ml_handler.drop_na(columns=['Date'], inplace=True)
